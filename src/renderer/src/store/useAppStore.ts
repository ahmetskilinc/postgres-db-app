...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setConnected: (id, connected) => {
    set((s) => ({
      connectedIds: connected
        ? s.connectedIds.includes(id)
          ? s.connectedIds
          : [...s.connectedIds, id]
        : s.connectedIds.filter((cid) => cid !== id)
    }))
  },
  ...
  toggleSchema: (connectionId, schema) => {
    set((s) => {
      ...
      if (!node) return {}
      const expanded = node.expandedSchemas.includes(schema)
        ? node.expandedSchemas.filter((sc) => sc !== schema)
        : [...node.expandedSchemas, schema]
      return {
        schemaStates: {
          ...s.schemaStates,
